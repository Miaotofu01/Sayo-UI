...
| 文件 | 大小 | 说明 |
|------|------|------|
| `sayo.css` | ~15 KB | 设计令牌（暗色 + 浅色）、reset、布局、组件、工具类 |
| `sayo.js` | ~17 KB | 交互引擎 — 光标、拖尾、波纹、视差、入场动画、Toast、数字跳动、惯性滚动、Dialog、Scroll Spy |
| `sayo.d.ts` | ~8 KB | TypeScript 类型定义 |
| `icons/` | 20 个 SVG | 自定义单线图标集 |
...
`icons/` 目录包含 20 个自定义单线图标，全部使用 `stroke="currentColor"` 继承文字颜色。预览见 `icons/index.html`。
